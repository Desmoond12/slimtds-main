...
/** @var list<\App\Admin\Repository\AffiliateNetwork> $networks */
/** @var string $app_url */
...
$renderSettings = function () use ($campaign, $action, $errors, $csrf_token, $titleKey, $networks) { ?>
<div style="max-width:560px">
    ...
    <form method="post" action="<?= e($action) ?>">
        ...
        <div class="form-section">
            <span class="form-section-label"><?= e(t('networks.title')) ?></span>
            <select id="network_id" name="network_id" class="input" style="max-width:340px">
                <option value=""><?= e(t('form.default')) ?></option>
                <?php foreach ($networks as $n): ?>
                    <option value="<?= e($n->id) ?>" <?= old('network_id', $campaign?->networkId ?? '') === $n->id ? 'selected' : '' ?>><?= e($n->name) ?></option>
                <?php endforeach; ?>
            </select>
            <p class="form-help"><?= e(t('networks.params_hint')) ?></p>
        </div>
        <?php endif; ?>
...
<div x-data="{ tab: (location.hash ? location.hash.slice(1) : 'settings') }"
    ...
    <p style="font-size:0.78rem;color:var(--color-faint);font-family:var(--font-sans);margin-top:10px">
        <?= e(t('campaigns.pixel_custom_events')) ?> <code style="background:var(--color-stone-100);padding:1px 5px;border-radius:2px;font-family:var(--font-mono);font-size:0.78rem">window.AffMomentTDS.track('purchase', {amount: 99})</code>
    </p>
    </section>
