...
<div style="max-width:560px">
    ...
    <form method="post" action="<?= e($action) ?>">
        ...
        <div class="form-section">
            ...
            <select id="network_id" name="network_id" class="input">
                <option value=""><?= e(t('form.default')) ?></option>
                <?php foreach ($networks as $n): ?>
                    <option value="<?= e($n->id) ?>" <?= old('network_id', $offer?->networkId ?? '') === $n->id ? 'selected' : '' ?>><?= e($n->name) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <?php endif; ?>
        ...
        <div class="form-section">
            ...
            <div>
                <?php
                    ...
                    // old() always returns a string (never null, even when passed a null default —
                    // see Shared/View/Helpers.php), so its own default param must carry the real
                    // fallback rather than being compared against null afterwards.
                    $existingUrls = $offer !== null ? $offer->postbackUrls : [];
                    $textareaValue = old('postback_urls_raw', implode("\n", $existingUrls));
                ?>
