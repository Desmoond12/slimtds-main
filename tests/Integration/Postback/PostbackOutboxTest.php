...
test('enqueue rawurlencodes macro values so they cannot inject extra query params', function (): void {
    $offer = $this->oRepo->create([
        'name' => 'OB Offer injection', 'url' => 'https://example.com/', 'is_active' => '1',
        'postback_urls' => ['https://network.example.com/cb?sub={click_id}&ext={external_id}'],
    ]);

    $n = $this->outbox->enqueue($this->conversionId, $offer->id, [
        'click_id'    => $this->clickId,
        'payout'      => '5.50',
        'status'      => 'approved',
        // Attacker-influenced value trying to append an unauthorized query param.
        'external_id' => 'x&admin=1&foo=bar',
        'currency'    => 'USD',
    ]);
    expect($n)->toBe(1);

    $row = $this->db->fetchOne(
        'SELECT target_url FROM core.postback_deliveries ORDER BY created_at DESC LIMIT 1',
    );
    $url = (string)$row['target_url'];
    expect($url)->toContain('ext=x%26admin%3D1%26foo%3Dbar');
