...
test('POST valid change revokes every other session but keeps the current one', function (): void {
    $aliceId = (int)$this->db->fetchScalar("SELECT id FROM core.admins WHERE login = 'alice'");

    // session_id() as a bare setter/getter works without session_start() —
    // it's enough to simulate "this is the id of the request making the change".
    session_id('current-session-under-test');
    $this->db->execute(
        "INSERT INTO core.sessions (id, admin_id, data, expires_at) VALUES ('current-session-under-test', :a, ''::bytea, now() + interval '1 day')",
        ['a' => $aliceId],
    );
    $this->db->execute(
        "INSERT INTO core.sessions (id, admin_id, data, expires_at) VALUES ('other-leaked-session', :a, ''::bytea, now() + interval '1 day')",
        ['a' => $aliceId],
    );

    $req = (new ServerRequestFactory())->createServerRequest('POST', '/admin/password');
    $req = $req->withParsedBody(['current' => 'oldpassword123', 'new_password' => 'freshpass2026', 'confirm' => 'freshpass2026']);
    $this->controller->post($req, new Response(), $this->view);

    $remaining = $this->db->fetchAll('SELECT id FROM core.sessions WHERE admin_id = :a', ['a' => $aliceId]);
    $ids = array_column($remaining, 'id');
    expect($ids)->toBe(['current-session-under-test']);

    session_id(''); // don't leak into other tests
});
