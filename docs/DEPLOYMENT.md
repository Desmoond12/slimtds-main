...
make migrate  # run Phinx migrations + create this/next month's partitions
...
`make migrate` also runs `partitions:rotate` — skipping it (or running raw `phinx migrate` yourself) leaves the partitioned `stats.*` tables with no partition for the current month, so the very first click/pixel event 500s with a Postgres check-violation. The cron container only rotates partitions once a day at 02:00 UTC, so on a fresh install that gap can last up to 24h if this step is missed — see `AI-INSTALL.md` §7 and its troubleshooting entry for this exact symptom.
...
**Trusted proxies**: The app reads the real visitor IP from `CF-Connecting-IP`. `config/frankenphp/Caddyfile.cf` carries a **static, hardcoded** list of Cloudflare IP ranges (`trusted_proxies static …` + `client_ip_headers CF-Connecting-IP`) — nothing refreshes it, so it needs a manual update if Cloudflare changes its ranges. `src/Shared/RealIp.php` independently mirrors the same Cloudflare range list and only walks `X-Slim-IP → X-Real-IP → CF-Connecting-IP → True-Client-IP → X-Forwarded-For` when `REMOTE_ADDR` itself falls inside that list (or inside `TRUSTED_PROXIES`); otherwise it uses `REMOTE_ADDR` directly and ignores every forwarding header. This is a defense-in-depth check independent of Caddy — it does not assume Caddy already stripped spoofed headers before they reach PHP.

**`TRUSTED_PROXIES`** in `.env.example` is only for an *extra* trusted proxy on top of the always-trusted Cloudflare ranges — leave it empty for `cf_flex`/`cf_full`. It matters for `direct` mode: see below.

**Firewall (do this for `cf_*` modes)**: nothing above stops someone from connecting to the origin IP directly and bypassing Cloudflare entirely if that IP leaks (DNS history, certificate-transparency logs for the CF Origin Cert, Shodan, a stray non-proxied `A` record). Restrict inbound 80/443 at the OS level to Cloudflare's published ranges:

```bash
# ufw example — refresh periodically, Cloudflare's ranges do change
sudo ufw default deny incoming
sudo ufw allow 22/tcp   # SSH — adjust/lock down separately
...
**IP trust in `direct` mode**: with no CDN in front, `REMOTE_ADDR` *is* the real visitor IP, and `src/Shared/RealIp.php` uses it directly — forwarding headers (`X-Forwarded-For` etc.) are ignored by default, since anyone on the internet could otherwise set them. If you put your own reverse proxy or load balancer in front of this Caddy instance, set `TRUSTED_PROXIES` in `.env` to that proxy's IP/CIDR, or its forwarded headers will be ignored too and every visitor will appear to come from the proxy's own IP (breaking GeoIP, bot-detection cloaking, and per-visitor rate limiting alike).
