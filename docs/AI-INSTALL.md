...
**Real client IP needs no configuration for `cf_*`/`dev`.** Leave `TRUSTED_PROXIES` empty. In
`cf_*` modes `config/frankenphp/Caddyfile.cf` already trusts Cloudflare's ranges and reads
`CF-Connecting-IP`; `src/Shared/RealIp.php` independently mirrors the same Cloudflare range list
and only honors `X-Slim-IP → X-Real-IP → CF-Connecting-IP → True-Client-IP → X-Forwarded-For`
when `REMOTE_ADDR` is itself inside that list — otherwise it uses `REMOTE_ADDR` directly, so a
direct connection to the origin IP (bypassing Cloudflare) cannot spoof its IP via headers.
Only set `TRUSTED_PROXIES` (comma-separated CIDR/IP) in `direct` mode, and only if you put your
own reverse proxy/load balancer in front of Caddy — otherwise leave it empty there too.
...
| Visitor IP is the proxy's IP | `DEPLOY_MODE` is not `cf_*` (no Cloudflare trust configured), and if you're behind your own reverse proxy in `direct` mode its IP isn't in `TRUSTED_PROXIES` | Set `DEPLOY_MODE=cf_flex`/`cf_full`, or add your proxy's IP/CIDR to `TRUSTED_PROXIES` in `direct` mode, restart |
