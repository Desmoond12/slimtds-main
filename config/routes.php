...
return static function (App $app): void {
    ...
    // Пиксель events (M2) — rate-limited per IP: campaign slugs/the pixel
    // script aren't secret, so this is a public, unauthenticated endpoint.
    // Preflight OPTIONS is exempt (no DB write, browsers send it freely).
    $app->post('/p/event', \App\Pixel\EventController::class)->add(PublicRateLimitMiddleware::class);
    $app->options('/p/event', [\App\Pixel\EventController::class, 'preflight']);

    // rrweb session chunks — same reasoning as /p/event.
    $app->post('/p/rec', \App\Pixel\RecordController::class)->add(PublicRateLimitMiddleware::class);
    $app->options('/p/rec', [\App\Pixel\RecordController::class, 'preflight']);

    // Постбэк (M2) — public token-authenticated endpoint, same reasoning.
    $app->get('/postback',  \App\Postback\PostbackController::class)->add(PublicRateLimitMiddleware::class);
    $app->post('/postback', \App\Postback\PostbackController::class)->add(PublicRateLimitMiddleware::class);
    ...
    $app->group('/admin', function (RouteCollectorProxy $g): void {
        ...
        // Affiliate networks — per-network postback param-name mapping
        $g->get('/networks',                     [\App\Admin\Controller\AffiliateNetworkController::class, 'all']);
        $g->get('/networks/new',                 [\App\Admin\Controller\AffiliateNetworkController::class, 'new_']);
        $g->post('/networks',                    [\App\Admin\Controller\AffiliateNetworkController::class, 'create']);
        $g->get('/networks/{id}/edit',            [\App\Admin\Controller\AffiliateNetworkController::class, 'edit']);
        $g->post('/networks/{id}',               [\App\Admin\Controller\AffiliateNetworkController::class, 'update']);
        $g->get('/networks/{id}/delete',          [\App\Admin\Controller\AffiliateNetworkController::class, 'deleteConfirm']);
        ...
        $g->post('/networks/{id}/reports/imports/{importId}/delete', [\App\Admin\Controller\PpReportController::class, 'deleteImport']);
        ...
        $g->get('/conversions/{id}/history', [\App\Admin\Controller\ConversionController::class, 'history']);

        // Raw postback audit trail + test-postback tool
        $g->get('/postback-log', [\App\Admin\Controller\PostbackLogController::class, 'index']);
