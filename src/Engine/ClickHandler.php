...
{
    /**
     * How schema_config['body'] macros are escaped before hitting the
     * response, keyed by schema_id — see MacroExpander::expand(). Schemas
     * not listed here either don't use 'body' or intentionally serve it raw
     * (ShowTextSchema: Content-Type text/plain, browsers won't execute it).
     */
    private const BODY_ESCAPE_MODE = [
        9  => 'html', // HtmlPageSchema
        11 => 'json', // JsonSchema
        14 => 'html', // HttpCodeSchema
        15 => 'html', // FormulaSchema
    ];
    ...
    public function handle(ServerRequestInterface $request, ResponseInterface $response, string $slug): ResponseInterface
    {
        ...
        if ($flow->targetType === 'offers' && $flow->targetOffers !== []) {
            ...
            // Paused/disabled offers left in target_offers (operator disabled
            // one leg of a split without editing the flow) must not still
            // receive traffic — strip them before weighted selection so the
            // remaining active offers' weights absorb the share correctly,
            // rather than the picker choosing a dead offer and its URL
            // being served anyway.
        ...
        // 'body' is escaped per destination context (see BODY_ESCAPE_MODE) —
        // several macros resolve to visitor-controlled data (referer, UA,
        // lander_*, utm_*) and must not be spliced raw into HTML/JSON.
        if (isset($schemaConfig['body'])) {
            $mode = self::BODY_ESCAPE_MODE[$schemaId] ?? 'raw';
            $schemaConfig['body'] = $this->macros->expand((string)$schemaConfig['body'], $ctx, $mode);
        }
    ...
    private function buildContext(ServerRequestInterface $request, string $slug): Context
    {
        ...
        // Only honored from a trusted proxy hop (same list RealIp uses, incl.
        // TRUSTED_PROXIES) — otherwise any direct client could set these
        // headers itself to poison attribution stats and, via {lander_host}/
        // {lander_domain}/{lander_button} macros, inject content into flow
        // response bodies.
        $trustedLander = RealIp::isRequestFromTrustedProxy($request);
        $landerHost = $trustedLander ? strtolower(trim($request->getHeaderLine('X-Lander-Host'))) : '';
        if ($landerHost !== '') {
        ...
        $landerPath = $trustedLander ? $request->getHeaderLine('X-Lander-Path') : '';
        if ($landerPath !== '') {
