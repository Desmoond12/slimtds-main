...
final class ConversionRepository
{
    ...
    /**
     * @param array{campaign_id?:?string, status?:?string, event_type?:?string, since?:?string} $filters
    ...
    /**
     * Status/payout transition log for one conversion, oldest first.
     * Populated by the core.conversion_status_history trigger — see
     * migrations/20260809000001_conversion_status_history.php.
     *
     * @return list<array<string,mixed>>
     */
    public function history(string $conversionId): array
    {
        return $this->db->fetchAll(
            'SELECT status, payout, external_id, changed_at
             FROM core.conversion_status_history
             WHERE conversion_id = :cid
