...
final class PasswordController
{
    ...
    public function post(ServerRequestInterface $request, ResponseInterface $response, View $view): ResponseInterface
    {
        ...
        // Revoke every other active session for this admin — a leaked
        // cookie must not survive a password change. The session doing the
        // change stays logged in (better UX than forcing a re-login right
        // after proving you know the new password).
        $this->db->execute(
            'DELETE FROM core.sessions WHERE admin_id = :admin_id AND id != :current_id',
            ['admin_id' => $admin->id, 'current_id' => (string)session_id()],
        );
