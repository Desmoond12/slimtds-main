...
final class PostbackOutbox
{
    ...
    private function fireOne(string $id, string $url, int $attempts): void
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_SSL_VERIFYPEER => true, // MITM protection — confirmed no partner uses self-signed certs
     ...
     *
     * Handles {payout}, {status}, {external_id}, {currency} directly via
     * str_replace, then delegates remaining tokens (e.g. {click_id},
     * {utm_*}) to MacroExpander. Every substituted value is rawurlencode'd
     * ('url' escape mode / explicit here for the direct tokens) since the
     * template is a URL and a value is spliced in as a query-string value —
     * `external_id` in particular can contain attacker-influenced data (see
     * PostbackController), and an unescaped `&`/`=` in it would let a
     * forged postback inject extra query parameters into the outbound
     * request sent to the partner network.
     *
    ...
    private function expand(string $template, array $vars): string
    {
        ...
        $direct = [
            '{payout}'      => rawurlencode((string)($vars['payout']      ?? '')),
            '{status}'      => rawurlencode((string)($vars['status']      ?? '')),
            '{external_id}' => rawurlencode((string)($vars['external_id'] ?? '')),
            '{currency}'    => rawurlencode((string)($vars['currency']    ?? '')),
        ];
        ...
        return $this->macros->expand($template, $ctx, 'url');
    }
}
