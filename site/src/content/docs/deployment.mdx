...
| `TRUSTED_PROXIES` | Extra trusted proxy IP/CIDR list, comma-separated, on top of the always-trusted Cloudflare ranges. Only needed in `direct` mode behind your own reverse proxy/load balancer, or if a lander's nginx proxies directly to slimTDS; see [Reverse Proxy](./reverse-proxy/) |
...
`App\Shared\RealIp` and `Caddyfile.cf` both only trust Cloudflare's published IP ranges for forwarded-IP headers — but nothing stops a client from connecting to the origin's public IP directly, bypassing Cloudflare, if that IP ever leaks (DNS history, certificate-transparency logs for a CF Origin Cert, Shodan, a stray non-proxied `A` record, a mail server sharing the box). Anyone who does can spoof `CF-Connecting-IP` with the header trust otherwise disabled at the origin, since a direct connection isn't from a Cloudflare range at all — but restricting inbound traffic at the OS level closes the door entirely and is worth doing for `cf_*` modes regardless:

```bash
sudo ufw default deny incoming
sudo ufw allow 22/tcp   # SSH — lock this down separately (key-only, non-default port, etc.)
...
Cloudflare's ranges change occasionally — re-run the loop above periodically (a monthly cron is enough) to stay current. This isn't needed for `direct` mode, where the origin is meant to be reachable by everyone.
