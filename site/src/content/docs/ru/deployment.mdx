...
| `TRUSTED_PROXIES` | Список дополнительных доверенных прокси (IP/CIDR через запятую) сверх всегда доверенных диапазонов Cloudflare. Нужен только в режиме `direct` за собственным reverse-proxy/балансировщиком, либо если nginx лендинга проксирует напрямую в slimTDS; см. [Обратный прокси](./reverse-proxy/) |
...
`App\Shared\RealIp` и `Caddyfile.cf` доверяют forwarded-IP заголовкам только из диапазонов Cloudflare — но ничто не мешает клиенту подключиться напрямую к публичному IP origin-сервера, минуя Cloudflare, если этот IP когда-либо утечёт (DNS-история, логи certificate-transparency для CF Origin Cert, Shodan, случайно оставленная непроксированная A-запись, почтовый сервер на том же боксе). Кто угодно, подключившись так, может подделать `CF-Connecting-IP` — доверие к заголовку на уровне origin в этом случае неприменимо, ведь прямое подключение вообще не из диапазона Cloudflare — но ограничение входящего трафика на уровне ОС закрывает эту дыру полностью, и стоит сделать это для режимов `cf_*` в любом случае:

```bash
sudo ufw default deny incoming
sudo ufw allow 22/tcp   # SSH — настройте отдельно (только по ключу, нестандартный порт и т.д.)
...
Диапазоны Cloudflare время от времени меняются — периодически (например, раз в месяц через cron) перезапускайте цикл выше, чтобы список оставался актуальным. Для режима `direct` это не нужно — там origin по замыслу доступен всем.
