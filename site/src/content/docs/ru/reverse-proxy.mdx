...
`App\Shared\RealIp` принимает заголовки выше только если непосредственный TCP-пир (`REMOTE_ADDR`) сам является доверенным прокси — диапазоны Cloudflare (доверяются всегда, захардкожены, синхронизированы с `Caddyfile.cf`) или диапазон, добавленный оператором через `TRUSTED_PROXIES`. Из любого другого источника заголовки полностью игнорируются, используется `REMOTE_ADDR` как есть. Проверка выполняется независимо в PHP, а не делегируется Caddy — она сработает, даже если собственная проверка `trusted_proxies` в Caddy почему-то будет обойдена.

**Если перед slimTDS стоит собственный nginx лендинга** (сценарии "first-party" выше) — это хоп прокси, которому slimTDS не доверяет автоматически. Добавьте его IP (или CIDR-блок, если у вас несколько лендинг-серверов) в `TRUSTED_PROXIES` в `.env`, иначе его `X-Real-IP` будет молча проигнорирован, и каждый посетитель за ним будет выглядеть так, будто пришёл с IP самого лендинг-сервера. Если перед этим nginx ещё стоит Cloudflare, её хоп уже доверяется автоматически — явно настраивать нужно только хоп лендинг-nginx.

Что ещё защищает цепочку:

- **Не выставляйте порт приложения в интернет.** Дотянуться до него должен только ваш прокси — привяжите порт к loopback, оставьте его внутри Docker-сети или закройте фаерволом. Это защита в глубину поверх проверки выше, а не замена ей.
- **В режимах `cf_*`** `config/frankenphp/Caddyfile.cf` задаёт `trusted_proxies static …` с захардкоженным списком диапазонов Cloudflare и `client_ip_headers CF-Connecting-IP`, поэтому Caddy принимает этот заголовок только от Cloudflare. Список статический — если Cloudflare изменит диапазоны, обновлять придётся вручную (тот же список захардкожен второй раз в `RealIp.php` — обновляйте оба вместе).
- **На уровне ОС/сети** ограничьте входящие 80/443 диапазонами Cloudflare при `cf_*` — см. [Развёртывание](./deployment/#firewall).
